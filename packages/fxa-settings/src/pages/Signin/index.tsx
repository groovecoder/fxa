...
const Signin = ({
  ...
  const isPocketClient = serviceName === 'Pocket';
  ...
  const localizedPocketAriaLabel = ftlMsgResolver.getMsg(
    'pocket-logo',
    'Pocket logo'
  );
  const localizedLogoAriaLabel = ftlMsgResolver.getMsg(
    'service-logo',
    `${serviceName} logo`,
    { serviceName }
  );
  ...
  // The avatar mixin is complex enough that I think it deserves its own ticket.
  ...
  const onSubmit = useCallback(async () => {
    try {
      ...
      // TODO: add message in Alert Bar re: success, then navigate to where appropriate
    } catch (e) {
      // TODO: metrics event for error
      // TODO: Add in localized Alert Bar message for error
      setError(e);
    }
  }, [
    email,
  ...
  // TODO:
  // Add in the Alert Bar in place of the original `success` and `error` display divs
    ...
    <>
        ...
        {isPasswordNeeded ? (
          ...
          <>
            ...
            <>
                ...
                <FtlMsg id="signin-subheader-with-logo">
                  <span className="card-subheader">
                    ...
                    <span>
                      <PocketLogo
                        className="inline"
                        aria-label={localizedPocketAriaLabel}
                      />
                    </span>
                  </span>
                </FtlMsg>
                ...
                <FtlMsg id="signin-subheader-with-logo">
                  <span className="card-subheader">
                    ...
                    <span>
                      <ServiceLogo
                        className="inline"
                        aria-label={localizedLogoAriaLabel}
                      />
                    </span>
                  </span>
                </FtlMsg>
              ...
              {!isPocketClient && !ServiceLogo && (
                <FtlMsg
                  id="signin-subheader-without-logo"
                  vars={{ serviceName }}
                >
                  <span className="card-subheader">
      ...
      <section>
        ...
        <div className="text-grey-500 my-5 text-xs">
          {isPocketClient ? (
            <FtlMsg id="firefox-and-pocket-tos">
              <p className="text-xs">
        ...
        <div className="flex justify-between">
          <FtlMsg id="use-a-different-account">
            <Link to="/" className="text-sm link-blue">
              Use a different account
            </Link>
          </FtlMsg>
          <FtlMsg id="forgot-password">
